...
def save_worktree(
    ...
    # snapshot the minidog package alongside the checkpoints so a run is reproducible
    pkg_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    shutil.copytree(pkg_dir, os.path.join(path, "minidog"), dirs_exist_ok=True,
                    ignore=shutil.ignore_patterns("__pycache__"))
    print(f"Source snapshot saved to {os.path.join(path, 'minidog')}")
